...
import (
	...
	commtls "github.com/ChinaArJun/fabric-sdk-go-gm/pkg/core/config/comm/tls"
	"github.com/ChinaArJun/fabric-sdk-go-gm/pkg/core/config/cryptoutil"
...
func detectDeprecatedNetworkConfig(endpointConfig *EndpointConfig) {
	...
	for _, v := range endpointConfig.networkConfig.Channels {
		if len(v.Orderers) > 0 {
			...
			logger.Warn("visit https://github.com/ChinaArJun/fabric-sdk-go-gm/blob/master/test/fixtures/config/overrides/local_entity_matchers.yaml for samples")
			break
		}
	}
}
